...
final class PCC_WooOTEC_Pro_Enroll {
    ...
    private function process_order(int $order_id, bool $force_email = false, bool $is_retry = false): bool {
        ...
        $order->update_meta_data('_pcc_moodle_user_id', $moodle_user_id);
    ...
    private function schedule_retry(WC_Order $order): void {
        ...
        $attempts++;
        $order->update_meta_data('_pcc_retry_attempts', $attempts);
        $order->save();
    ...
    private function send_order_access_email(WC_Order $order, bool $force = false, ?array $learner = null, ?string $password = null, ?array $urls = null): bool {
        if (PCC_WooOTEC_Pro_Core::instance()->get_option('email_enabled', 'yes') !== 'yes') {
            return false;
        }
        ...
        if ($sent) {
            ...
            $order->save();
            PCC_WooOTEC_Pro_Logger::info('Correo de acceso enviado', array('order_id' => $order->get_id(), 'email' => $learner['email']));
            return true;
        }

        PCC_WooOTEC_Pro_Logger::error('Fallo envio de correo de acceso', array('order_id' => $order->get_id(), 'email' => $learner['email']));
        return false;
    }
